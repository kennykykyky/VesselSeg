...
class UNetModel(nn.Module):
    ...
    def forward(self, data):
        ...
        self.seg_pred = (self.net.decoder.outputs['seg'] > 0.).float() # [B, 1, ...]
        return output

    def loss_focal(self):
        loss = self.focal_loss(self.net.decoder.outputs['seg'], self.segs_gt[:, 0, None])
        return loss

    def before_epoch(self, mode='train'):
    ...
    def get_metrics(self, data, output, mode='train'):
        ...
        with torch.no_grad():
            seg_gt = self.segs_gt.type(torch.int64)[:, 0, None] # [B, 1, ...]
    ...
    def vis(self, writer, global_step, data, output, mode, in_epoch):
        if mode == 'train':
            ...
            if not in_epoch:
                return
            if self.cfg.var.case_ves != self.case_ves_idx_png_vis[0]:
                return
        with torch.no_grad():
            b = self.case_ves_idx_png_vis[1]
            n_rows = 1
            imgs_show = OrderedDict()
            imgs_show['imgs'] = self.imgs[b, :, None]

            if self.imgs[b].shape[0] == 1:
                padding = 0
            else:
                padding = 2
            for key, img in imgs_show.items():
                imgs_show[key] = make_grid(img * 255, nrow=img.shape[0], pad_value=100,
                                           padding=padding).cpu().numpy()[0]

            fig, axes = plt.subplots(
                n_rows, 1, figsize=(0.4 * max(len(self.cfg.dataset.mods_val), 3) * self.cfg.dataset.hw_img / 20,
                                    self.cfg.dataset.hw_img / 25 + 2))
            if not hasattr(axes, 'reshape'):
                axes = [axes]
            for ax, key in zip(axes, imgs_show.keys()):
                ax.axis('off'), ax.set_xticks([]), ax.set_yticks([]), ax.set_title(key, fontsize='14')
                ax.imshow(imgs_show[key], cmap='gray')
            fig.suptitle(f'{self.case_ves_idx_png_vis[0]}, {self.case_ves_idx_png_vis[1]:03d}.png')
            fig.tight_layout(pad=0.01)

            for i in range(self.imgs[b].shape[0]):
                conts = measure.find_contours(self.segs_gt[b, i].cpu().numpy())
                for cont in conts:
                    axes[0].plot(cont[:, 1] + (padding + self.cfg.dataset.hw_img) * i + padding, cont[:, 0] + padding,
                                 linewidth=1, color='#0099ff')
                conts = measure.find_contours(self.seg_pred[b, 0].cpu().numpy())
                for cont in conts:
                    axes[0].plot(cont[:, 1] + (padding + self.cfg.dataset.hw_img) * i + padding, cont[:, 0] + padding,
