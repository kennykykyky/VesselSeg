...
"""
class TopoInter_Loss(torch.nn.Module):
    def __init__(self, dim, connectivity, inclusion, exclusion, min_thick=1):
        ...
        """
        super(TopoInter_Loss, self).__init__()
...
if __name__ == "__main__":
    ...
    if dim == 2:
        ...
        ti_loss_weight = 1e-4
        ti_loss_func = TopoInter_Loss(dim=2, connectivity=4, inclusion=[[1,2]], exclusion=[[2,3],[3,4]])
        ...
        ti_loss_weight = 1e-6
        ti_loss_func = TopoInter_Loss(dim=3, connectivity=26, inclusion=[[1,2]], exclusion=[[2,3],[3,4]], min_thick=1)
        ti_loss_value = ti_loss_func(x, y) if ti_loss_weight != 0 else 0
        ti_loss_value = ti_loss_weight * ti_loss_value
        print("ti_loss_value: ", ti_loss_value)