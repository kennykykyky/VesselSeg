...
def generate_detection_train_transform(
    ...
    train_transforms = Compose(
        [
            LoadImaged(keys=[image_key, seg_key], ensure_channel_first=True, image_only=False),
            ...
            Orientationd(keys=[image_key, seg_key], axcodes="LPS"),
            intensity_transform,
...
def generate_detection_inference_transform(
    ...
    test_transforms = Compose(
        [
            LoadImaged(keys=[image_key, seg_key], meta_key_postfix="meta_dict"),
            # LoadImaged(keys=[image_key], meta_key_postfix="meta_dict"),
            EnsureChannelFirstd(keys=[image_key]),
            EnsureTyped(keys=[image_key], dtype=torch.float32),
            Orientationd(keys=[image_key, seg_key], axcodes="RAS"),
            intensity_transform,
